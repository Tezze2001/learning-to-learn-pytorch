...
if not os.path.isfile(f'{path_results}/results_mnist_number_neurons_test.npz'):
    NORMAL_OPTS = [(optim.Adam, {}), (optim.RMSprop, {}), (optim.SGD, {}), (optim.SGD, {'nesterov': True, 'momentum': 0.9})]
    OPT_NAMES = ['ADAM', 'RMSprop', 'SGD', 'NAG']
    #n_neurons = [5, 10, 20, 25, 30] old_1
    #n_neurons = [5, 10, 20, 30, 40, 50] old_2
    n_neurons = [60, 70, 100]

    QUAD_LRS = [0.01, 0.01, 0.1, 0.1]
    fit_data_mnist = np.zeros((100, 100, (len(OPT_NAMES) + 1)* len(n_neurons)))
    for i, ((opt, extra_kwargs), lr) in enumerate(zip(NORMAL_OPTS, QUAD_LRS)):
        for j, n in enumerate(n_neurons):
            torch.manual_seed(42)
            MNISTNet_partial = partial(MNISTNet, layer_size=n)
            fit_data_mnist[:, :, i * len(n_neurons) + j] = np.array(fit_std_optimizer(MNISTLoss, MNISTNet_partial, opt, lr=lr, **extra_kwargs))
    ...
    for j, n in enumerate(n_neurons):
        torch.manual_seed(42)
        MNISTNet_partial = partial(MNISTNet, layer_size=n)
        fit_data_mnist[:, :, len(OPT_NAMES) * len(n_neurons) + j] = np.array([fit_outer_model(opt, None, MNISTLoss, MNISTNet_partial, unroll=1, n_steps_to_optimize=100, out_mul=0.1, should_train=2) for _ in range(100)])


    np.savez(f'{path_results}/results_mnist_number_neurons_test.npz', data=fit_data_mnist)
else: 
    fit_data_mnist = np.load(f'{path_results}/results_mnist_number_neurons_test.npz')['data']
...
def plot_performance(name_optimizers, number_neurons, fit_data, title_plot, n_epochs=100, n_optimization_steps=100):
    ...
    for l, n_neuron in enumerate(number_neurons):
        ...
        plt.xlabel('Steps')
        plt.ylabel('Loss')
        plt.title(f"Optimization performance of FFNN with 1 hidden layer with {n_neuron} neurons executed on mnist")
        plt.legend()
        plt.show()
    
plot_performance(['ADAM', 'RMSprop', 'SGD', 'NAG', 'LSTM'], [60, 70, 100], fit_data_mnist, "varing number of neurons")