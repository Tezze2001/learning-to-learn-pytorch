...
n_layers = 1
activation = [nn.Sigmoid(), nn.ReLU(), nn.LeakyReLU(), nn.Tanh()]
layer_size = 20
...
if not os.path.isfile(f'{path_results}/results_mnist_activation_test.npz'):
    ...
    fit_data_mnist = np.zeros((100, 100, (len(OPT_NAMES) + 1)* len(activation)))
    for i, ((opt, extra_kwargs), lr) in enumerate(zip(NORMAL_OPTS, QUAD_LRS)):
        for j, act in enumerate(activation):
            torch.manual_seed(42)
            MNISTNet_partial = partial(MNISTNet, layer_size=layer_size, n_layers=n_layers, activation= act)
            fit_data_mnist[:, :, i * len(activation) + j] = np.array(fit_std_optimizer(MNISTLoss, MNISTNet_partial, opt, lr=lr, **extra_kwargs))
    ...
    for j, act in enumerate(activation):
        torch.manual_seed(42)
        MNISTNet_partial = partial(MNISTNet, layer_size=layer_size, n_layers=n_layers, activation= act)
        fit_data_mnist[:, :, len(OPT_NAMES) * len(activation) + j] = np.array([fit_outer_model(opt, None, MNISTLoss, MNISTNet_partial, unroll=1, n_steps_to_optimize=100, out_mul=0.1, should_train=2) for _ in range(100)])


    np.savez(f'{path_results}/results_mnist_activation_test.npz', data=fit_data_mnist)
else: 
    fit_data_mnist = np.load(f'{path_results}/results_mnist_activation_test.npz')['data']
    print('risultati caricati da file')

def plot_performance(name_optimizers, number_neurons, fit_data, n_epochs=100, n_optimization_steps=100):
    ...
    for l, n_neuron in enumerate(number_neurons):
        ...
        plt.xlabel('Steps')
        plt.ylabel('Loss')
        plt.title(f"Optimization performance of FFNN with {n_neuron} as activation function executed on mnist")
        plt.legend()
        plt.show()
    
plot_performance(['ADAM', 'RMSprop', 'SGD', 'NAG', 'CLSTMOpt'], activation, fit_data_mnist)